...
import { auditShifts, getAuditSummary, RULES } from "./arbzg";
...
export default function App() {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured) {
      try {
        localStorage.setItem(DEMO_STORAGE.employees, JSON.stringify(employees));
        localStorage.setItem(DEMO_STORAGE.shifts, JSON.stringify(rawShifts));
      } catch {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    setDataLoading(true);
    Promise.all([fetchOrganizationData(profile.organization_id), fetchOrganizationRules(profile.organization_id)])
      .then(([{ employees: nextEmployees, shifts: nextShifts }, nextRules]) => {
        if (!mounted) return;
  ...
  const auditedShifts = useMemo(() => auditShifts(rawShifts, rules), [rawShifts, rules]);
