...
export default function ComplianceSettingsPage({ organizationId, userId, onRulesSaved }) {
  ...
  useEffect(() => { let active = true; fetchOrganizationRules(organizationId).then((data) => active && data && setRules(data)).catch((err) => active && setError(err.message)).finally(() => active && setLoading(false)); return () => { active = false; }; }, [organizationId]);
  function update(key, value) { setRules((current) => ({ ...current, [key]: Number(value) })); }
  async function save(event) { event.preventDefault(); setSaving(true); setMessage(""); setError(""); try { const saved = await updateOrganizationRules(rules, organizationId, userId); setRules(saved); onRulesSaved?.(saved); setMessage(t("settings.saved")); } catch (err) { setError(err.message); } finally { setSaving(false); } }
